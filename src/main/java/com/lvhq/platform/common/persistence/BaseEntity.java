...
public abstract class BaseEntity<T> implements Serializable {
	...
	private static String computerCode = null;

	static {
		try {
			computerCode = (Math.abs(InetAddress.getLocalHost().getHostName().hashCode()) + "00").substring(0, 2);
		} catch (UnknownHostException e) {
			computerCode = new Random().nextInt(100) + "";
		} catch (Exception e) {
			computerCode = new Random().nextInt(100) + "";
		}
	}
	...
	static String l6() {
		...
		sBuilder.append(computerCode);
		sBuilder.append(Thread.currentThread().getId());
		if (sBuilder.length() < 6) {
			for (int i = sBuilder.length(); i < 6; i++) {
				sBuilder.append("0");
			}
		}
