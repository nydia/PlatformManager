package com.lvhq.platform.common.servlet;
...
public class ValidateCodeServlet extends HttpServlet {
	...
	private void createBackground(Graphics2D g2d) {
		// 填充背景
		g2d.setColor(getRandColor(220, 250));
		g2d.fillRect(0, 0, w, h);
		// 加入干扰线条
		for (int i = 0; i < 10; i++) {
			g2d.setColor(getRandColor(40, 150));
			Random random = new Random();
	...
	private String createCharacter(Graphics2D g2d) {
		char[] codeSeq = { 'A', 'B', 'C', 'D', 'E', 'F', 'G', 'H', 'J', 'K', 'M', 'N', 'P', 'Q', 'R', 'S', 'T', 'U',
				'V', 'W', 'X', 'Y', 'Z', '2', '3', '4', '5', '6', '7', '8', '9' };
		String[] fontTypes = { "Arial", "Courier New", "Consolas", "Calibri" };
		Random random = new Random();
		StringBuilder s = new StringBuilder();
		for (int i = 0; i < 4; i++) {
			String r = String.valueOf(codeSeq[random.nextInt(codeSeq.length)]);// random.nextInt(10));
			g2d.setColor(new Color(50 + random.nextInt(100), 50 + random.nextInt(100), 50 + random.nextInt(100)));
			g2d.setFont(new Font(fontTypes[random.nextInt(fontTypes.length)], Font.BOLD, 26));
			g2d.drawString(r, 20 * i + 15, 25 + random.nextInt(8));
